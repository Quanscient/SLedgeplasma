...
plasma_flow += integral(SOL, (dof(N)*grad(M) + M*grad(dof(N))
                            ...
                            -  N*grad(M) - M*grad(N)) * tf(N), 0, 1 );    // 1st term: Linearization of divergence term
plasma_flow += integral(SOL, -Sin * tf(N), 0, 1);                         // 2nd term: Source term
...
with: 2/(m_i*c_s²) = 1/Tt
---------------------------------------------------------------------------------------*/
plasma_flow += integral(SOL, 2*(dof(N)*grad(M)*M + N*grad(M)*dof(M) + N*grad(dof(M))*M - 2*N*grad(M)*M) * tf(M), 0, 1);    // Linearization of 1st term
plasma_flow += integral(SOL, (M*M*grad(dof(N)) + 2*M*dof(M)*grad(N) - 2*M*M*grad(N) ) * tf(M), 0, 1 );    // Linearization of 2nd term
...
) * tf(M), 0, 1 );    // 3rd term Linearized
plasma_flow += integral(SOL, -dofSmn * tf(M), 0, 1);     // 4th term: zero for 2point model
...
if (LSIC == true){ plasma_flow += integral(SOL, Pw_lsic * tau * Rc_lsic, 0, 1); }
if (SUPG == true){ plasma_flow += integral(SOL, Pw_supg * tau * Rm_supg, 0, 1); }
if (PSPG == true){ plasma_flow += integral(SOL, Pq_pspg * tau * Rm_pspg, 0, 1); }
