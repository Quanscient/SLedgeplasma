...
int main(void){
    ...
    //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    #include "preprocess_1D_SOL_case-04.h"
    ...
    double massflux_upstream = 0.0, momnetumflux_upstream = 0.0, energyflux_upstream = 0.0, neutralflux_upstream = 0.0;
    double massflux_target   = 0.0, momnetumflux_target   = 0.0, energyflux_target   = 0.0, neutralflux_target   = 0.0;
    double massflux_balance  = 0.0, momnetumflux_balance  = 0.0, energyflux_balance  = 0.0, neutralflux_balance  = 0.0;
    ...
    while (q_rampeddown==false)
    {
        ...
        while( isconverged_CS==false && iter_CS<maxiters_CS ){
            ...
            // if (rampedup==false)
            //     plasma_flow.solve("lu", false, {1,2}); 
            // else
            //     plasma_flow.solve("lu", false); 
            ...
            if(rampedup==false){
                ...
                Nn.setvalue(selectall(), max(1e-20, Nn));
            }
            ...
            if (log_file.is_open()){
                log_file << std::scientific << std::setprecision(1) << q_next         << "," \
                            ...
                            << std::scientific << std::setprecision(1) << Nn_upstream << "," \
                            << std::scientific << std::setprecision(5) << N_target    << "," \
                            << std::scientific << std::setprecision(1) << M_target    << "," \
                            << std::scientific << std::setprecision(1) << T_target    << "," \
                            << std::scientific << std::setprecision(5) << Nn_target   << std::endl;
            }
            else {
                std::cout << "Unable to open the log file" << std::endl;
            }
            ...
            expression energyflux = beta*on(SOL, -K*grad(T)) + alpha*((5.0/2.0)*T*N*M*n_o*c_st)*e;
            ...
            if(rampedup==true){
                ...
                if(isconverged_CS==false){
                    ...
                    T.setconstraint(target, Spn.integrate(SOL, 5)/(gamma_se*GAMMA_se*e));
                ...
                // double max_beta = 1e-6; // move this to preprocess file
                // if(isconverged_Tt && !maxbeta_reached){
                //     // a = a + 0.05;
                //     // alpha.setvalue(selectall(), min(a, 1.0));
                //     // b = b - 0.05; if(b-max_beta<1e-6){ maxbeta_reached=true; }
    ...
    sigVrc.write(selectall(), "sigVrc.vtu", 2);
    sigViz.write(selectall(), "sigViz.vtu", 2);
