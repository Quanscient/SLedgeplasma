...
double N_start=0.01, N_next=N_start, N_end=0.09269695813007907;    // N_end = 1.0 --> n = n_o at upstream # seems to help in convergence
double q_start=qsol, q_next=q_start, q_end=qsol;


/*
Nend=0.11219029035652166, fluxsource=1.6e+22*L=4e23                  for qcond
Nend=0.09318585319279925, fluxsource=1.6e+22*L                       for qconv
Nend=0.09318585319279925, fluxsource=1.6e+22*L                       for qconv_sheath11
Nend=0.09994008413997456 (0.1), fluxsource=1.4668104681923027e+22*L  for qcond_densityupstream 
*/
/* ------------------------------------------------------------------------------------------------------ */
...
//++++++++++++
const double particleflux = 1.6e+22 * L; //1.6e+22*L=4e23;   // [1/(m²s)]
expression particlesource = (particleflux / L);  // [1/(m³s)]

const double energyflux = qsol;     // [W/m²]
expression   energysource = (energyflux / L);  // [J/(m³s)] or [W/m³]
